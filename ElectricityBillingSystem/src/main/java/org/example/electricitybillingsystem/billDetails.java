...
public class billDetails extends Application {
    @FXML
    private TextField searchMeterTextField, searchStatusTextField;
    ...
    public void start(Stage primaryStage) {
        try {
            ...
            e.printStackTrace();
            showAlert("Error", "Unable to load the Bill Details screen.");
        }
    }
    ...
    private void initialize() {
        ...
        searchStatusTextField.setEditable(true);
        searchStatusTextField.setFocusTraversable(false);
    ...
    private void handleActionButton(ActionEvent event) {
        if (event.getSource() == search) {
            ...
            String enteredStatus = searchStatusTextField.getText();

            String querySearch;

            if (enteredMeter != null && !enteredMeter.isEmpty()) {
                querySearch = "SELECT * FROM bill WHERE meter_no = '" + enteredMeter + "'";
            } else if (!enteredStatus.isEmpty()) {
                querySearch = "SELECT * FROM bill WHERE status = '" + enteredStatus + "'";
            } else {
                showAlert("Error", "Please enter a meter number or status for search.");
                return;
            }
